...
void PrimaryGenerator::PrimaryFromSphere(G4Event *anEvent)
{
...
void PrimaryGenerator::PrimaryFromBeam(G4Event *anEvent)
{
    // Define a partícula (mu-)
    G4ParticleDefinition* particle = G4ParticleTable::GetParticleTable()->FindParticle("mu-");
    fParticleGun->SetParticleDefinition(particle);
    ...
    G4double r = 1.5 * m;
    G4double theta_pos = std::acos(G4RandFlat::shoot(0.99, 1.0));  // [0, pi]
    G4double phi_pos = G4RandFlat::shoot(0., 2. * CLHEP::pi);      // [0, 2pi]
