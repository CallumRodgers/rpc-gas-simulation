...
int main(int argc, char* argv[])
{
	...
	{	
		...
		if(ports[0])
		{
			uint64_t last=(*(uint64_t*)(readarray1+8));
			for(int i=2;i<(hod_size+1);i++) //get the first fifo position in this port to have 1s in non-stuck pads
			{
				uint64_t pad=(*(uint64_t*)(readarray1+i*8));
				uint64_t newbits=(pad^last)&(~last);
				if(newbits){pads1=newbits;break;}
			}
		}
		if(ports[1])
		{
			uint64_t last=(*(uint64_t*)(readarray2+8));
			for(int i=2;i<(hod_size+1);i++) //get the first fifo position in this port to have 1s in non-stuck pads		
			{
                                uint64_t pad=(*(uint64_t*)(readarray2+i*8));          
                                uint64_t newbits=(pad^last)&(~last);
                                if(newbits){pads2=newbits;break;}
			}
		}
		if(ports[2])
		{
			uint64_t last=(*(uint64_t*)(readarray3+8));
			for(int i=1;i<(hod_size+1);i++) //get the first fifo position in this port to have 1s in non-stuck pads
			{
                                uint64_t pad=(*(uint64_t*)(readarray3+i*8));
                                uint64_t newbits=(pad^last)&(~last);
                                if(newbits){pads3=newbits;break;}
			}
		}
		if(ports[3])
		{
			uint64_t last=(*(uint64_t*)(readarray4+8));
			for(int i=1;i<(hod_size+1);i++) //get the first fifo position in this port to have 1s in non-stuck pads
			{
                                uint64_t pad=(*(uint64_t*)(readarray4+i*8));
                                uint64_t newbits=(pad^last)&(~last);
                                if(newbits){pads4=newbits;break;}
			}
		}
